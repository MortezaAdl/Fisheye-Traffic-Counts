...
from counter.Intersection import ImgCX
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        h, w = img0.shape[0:2]
        if w > h:
            img0c = img0[:, int(ImgCX - h/2):int(ImgCX + h/2)]
            displacements = int(ImgCX - h/2)
        else:
            img0c = img0
            displacements = 0
        
        # Padded resize
        img = letterbox(img0c, self.img_size, stride=self.stride)[0]

        # Convert
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img = np.ascontiguousarray(img)

        return path, img, img0, self.cap, displacements
